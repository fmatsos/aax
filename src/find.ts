...
const SEARCH_API_BASE = process.env.AAX_API_URL || 'https://aax.sh';
...
${DIM}  2) npx aax add <owner/repo@skill>${RESET}`;
  ...
  if (query) {
    ...
    for (const skill of results.slice(0, 6)) {
      ...
      console.log(`${DIM}└ https://aax.sh/${skill.slug}${RESET}`);
      console.log();
    }
    return;
  }
  ...
  if (info && (await isRepoPublic(info.owner, info.repo))) {
    console.log(`${DIM}View the skill at${RESET} ${TEXT}https://aax.sh/${selected.slug}${RESET}`);
  } else {
    console.log(`${DIM}Discover more skills at${RESET} ${TEXT}https://aax.sh${RESET}`);
  }
